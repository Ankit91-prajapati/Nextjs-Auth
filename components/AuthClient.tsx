...
export default function AuthClientPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignIn) {
        ...
        if (!result.user) {
          ...
          // After signup, immediately log the user in and redirect
          const loginResult = await signIn(email, password);
          if (loginResult.user) {
            router.refresh();        // refresh session
            router.push("/dashboard"); // redirect
          } else {
            setError("Signup successful, but login failed. Please sign in.");
          }
        }
      }
      } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-t from-blue-400 to-indigo-200">
